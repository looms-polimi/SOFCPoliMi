...
partial model IdealMixture
  extends PartialMixture(
    nX = 8,
    nC = 0,
    Xi_start = X_start[1:nXi],
    nXi = if reducedX then nX-1 else nX,
    rho(start = rho_start),
    computeEnthalpyCondensation = false,
    computeTransport = false,
    computeEntropy = false,
    du_dT(nominal=1000),
    du_dX(each nominal=1e6),
    dv_dp(nominal=1e-8),
    dv_dT(nominal=1e-4),
    dv_dX(each nominal=0.1));
...
//   parameter Types.Temperature T_start "";
  ...
  Types.MolarMass MMmix(start = 0.003) "Molar mass of the mixture";
  Types.SpecificEntropy s_species[nX] "";
  Types.SpecificVolume v(start=v_start) "Specific volume of rhe mixture";
...
equation
  ...
  MMmix = MMmixEval(X); // "Molar mass of the mixture"; 0.0197049
  ...
  dv_dp = -v/p;//-R/MMmix*T/p^2 "Partial derivative definition";
