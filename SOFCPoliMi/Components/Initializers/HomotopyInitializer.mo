...
model HomotopyInitializer
  ...
  replaceable model Medium = Media.MainClasses.SOS10Components constrainedby
    Media.BaseClasses.PartialMixture
  ...
  Medium refFluid(
      ...
      T_start = T_start,
      X_start = X_start,
      computeEnthalpyCondensation = computeEnthalpyCondensation);
  SOFCPoliMi.Interfaces.FlangeA inlet(nXi = refFluid.nXi, nC = refFluid.nC) annotation (
  ...
  SOFCPoliMi.Interfaces.FlangeB outlet(nXi = refFluid.nXi, nC = refFluid.nC) annotation (
...
equation
  ...
  outlet.h = homotopy(inStream(inlet.h), h_start);
