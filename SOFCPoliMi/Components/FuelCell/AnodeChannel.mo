...
model AnodeChannel
  extends Components.FuelCell.BaseChannel(isAnode=true, alphaNom = 510, Nu=3.6);
  ...
  // rRates model
  HOR hor(T=htPEN.Tw, T_start=T_start);
  ReactionRates rRates(T=Tout, pX=pX, T_start=T_start, Tnom=Tnom, nX=nX);
...
//   dM_dt = -V*rhoOut^2*(fluidOut.dv_dT*der(Tout)+fluidOut.dv_dp*der(pOut)+ fluidOut.dv_dX*der(Xout));
  ...
  M*der(Xout) = wIn*(Xin-Xout) + S*(rHOR*massStoichHOR + rSR*massStoichSR + rWGS*massStoichWGS) - Xout*wO2 + {0,0,0,0,0,0,0,wO2,0,0};
//   M*der(Xout) + Xout*dM_dt = wIn_i - wOut_i + dM_dt


  checkConsumption = S*(rHOR*massStoichHOR + rSR*massStoichSR + rWGS*massStoichWGS);
  checkConsH2 = S*(rHOR*massStoichHOR);
  balance = wIn_i - wOut_i + S*(rHOR*massStoichHOR + rSR*massStoichSR + rSR*massStoichSRC2H6 + rSR*massStoichSRC3H8 + rWGS*massStoichWGS) + {0,0,0,0,0,0,0,wO2,0,0};
  consHOR = S*rHOR*massStoichHOR;
  consWGS = S*rWGS*massStoichWGS;
  consSR = S*rSR*massStoichSR;
