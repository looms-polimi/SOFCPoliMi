...
partial model BaseChannel
  ...
  // Initialization
  type InitType = enumeration(
      steadyState "Steady state initialization",
      fixedValues "Fixed values initialization");
  ...
  parameter Types.Pressure pX_startWithZeros[nX] = p_start*X_start.*fluidIn.MM/fluidIn.MM[fluidIn.dominantSpecies];
  parameter Types.Pressure pX_start[nX] = {max(pX_startWithZeros[i],ModelicaServices.Machine.eps) for i in 1:nX};
  parameter Types.Density rho_start = 8 "Start value for density";
  parameter Types.Temperature T_start = 900 "Start value for temperature";
  parameter Types.MassFraction X_start[nX] = ones(nX)/nX "Start value for composition";
  parameter Types.MassFlowRate w_start = 0.000117298e-3 "Start value for mass flow rate";
  ...
  Types.Pressure pOut( start = p_start, nominal=101325) "Mixture outlet pressure";
  Types.Pressure pX[nX](start = pX_start) "Partial pressures in the channel";
  Types.Density rhoIn(start = rho_start) "Mixture inlet density";
  Types.Density rhoOut(start = rho_start) "Mixture outlet density";
  Types.Temperature Tin(start = T_start, nominal=1e3) "Mixture inlet temperature"; // , nominal=1000
  Types.Temperature Tout(start = T_start, nominal=1e3) "Mixture outlet temperature"; // , nominal=1000
  Types.Temperature TwPEN(start=T_start, nominal=1e3) "Temperature of the PEN (wall)"; // , nominal=1000
  Types.Temperature TwPlate(start=T_start, nominal=1e3) "Temperature of the interconnection plate (wall)"; // , nominal=1000
  Types.MassFlowRate wIn(start=w_start, nominal=1e-6) "Channel inlet mass flow rate";
  Types.MassFlowRate wIn_i[nX] = wIn*Xin "";
  Types.MassFlowRate wOut(start=w_start, nominal=1e-6) "Channel outlet mass flow rate";
  Types.MassFlowRate wOut_i[nX] = wOut*Xout "";
  Types.MassFlowRate wO2(start = 0, nominal=1e-12) "Oxygen flow from cathode channel to anode channel";
  Types.MassFraction Xin[nX](start = X_start) "Mixture inlet mass fractions";
  Types.MassFraction Xout[nX](start = X_start) "Mixture outlet mass fractions";
  ...
  // Media variables
  Fluid fluidIn(nX = nX, anodicFlow = isAnode, cathodicFlow = not isAnode, T_start=T_start, p_start=p_start, X_start=X_start, rho_start=rho_start) "Fluid at channel inlet";
  Fluid fluidOut(nX = nX, anodicFlow = isAnode, cathodicFlow = not isAnode, T_start=T_start, p_start=p_start, X_start=X_start, rho_start=rho_start) "Fluid at channel outlet";

  // Fluid and Heat ports
  Interfaces.FlangeA inlet(nXi = nX, nC = 0) "Inlet connector"  annotation (
  ...
  Interfaces.FlangeB outlet(nXi = nX, nC = 0) "Outlet connector"  annotation (
  ...
  HeatTransfer htPEN(Nu=Nu, Lc=Di, A=A, S=S, Tw=TwPEN, Tflow=Tout, k=fluidIn.k, T_start=T_start, alphaNom = alphaNom) ""; //(Tout+Tin)/2
  HeatTransfer htPlate(Nu=Nu, Lc=Di, A=A, S=Sintercon, Tw=TwPlate, Tflow=Tout, k=fluidIn.k, T_start=T_start, alphaNom = alphaNom) "";
  // (Tout+Tin)/2
...
equation
  ...
  inlet.Xi = X_start "Dummy never used: no flow reversal";
  // O2 ion transport
  wO2 = 0.5 * rHOR * MMO2 * S;
  hO2 = fluidOut.h_species[6];
